...
class TaskCreateViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func Done() {
        ...
        else if start!.earlierDate(phaseStart) == start {
            alertController.message = "Start date cannot be before phase start date."
        }
        else if deadline!.laterDate(phaseDeadline) == deadline {
            alertController.message = "Deadline cannot be after phase deadline."
        }
